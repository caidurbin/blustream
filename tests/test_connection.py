...
class TestTCPConnection:
    ...
    async def test_connect_success(self, mock_open_connection):
        ...
        conn = TCPConnection(host="192.0.2.100", port=23)
        await conn.connect()
    ...
    async def test_no_banner_proceeds(self, mock_open_connection):
        ...
        conn = TCPConnection(host="192.0.2.100", port=8000, banner_timeout=0.05)
        await conn.connect()
        assert conn.is_connected()
    ...
    async def test_banner_then_immediate_command_response(self, mock_open_connection):
        ...
        mock_reader.read = AsyncMock(side_effect=[full_banner, "[SUCCESS]ok\r\n"])
        mock_open_connection.return_value = (mock_reader, mock_writer)
...
class TestStreamingResponseFraming:
    ...
    @pytest.mark.asyncio
    @patch("telnetlib3.open_connection")
    async def test_banner_broadcast_in_status_stream_is_stripped(
    ...
    async def test_back_to_back_commands_do_not_bleed(self, mock_open_connection):
        ...
        assert "[SUCCESS]" not in first, (
            "tail of next response must NOT leak into first"
        )
