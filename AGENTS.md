...
When you change any file under `.github/` (a workflow, `actionlint.yaml`, or a composite action), run `actionlint` before finishing and resolve everything it reports. It validates the Actions schema, type-checks `${{ }}` expressions, and runs shellcheck over `run:` blocks — catching mistakes a plain YAML parse can't. CI enforces the same check via `.github/workflows/actionlint.yml`.
