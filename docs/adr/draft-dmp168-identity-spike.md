...
Spike against firmware **MCU V1.5.0 / Web V1.4.0 / DSP V1.5.9** at `192.168.1.176` on 2026-05-27. Goal: determine whether the Home Assistant custom integration can read a stable hardware identifier (MAC / serial / hostname) from the device over the network.
...
| `_ssh._tcp.local.` | `DMP168._ssh._tcp.local.` | `DMP168.local.` | 22 | *(none)* |
...
- A-record resolves cleanly: `DMP168.local → 192.168.1.176`.
- No custom `_blustream._tcp` / `_dmp168._tcp` service type. No TXT field carries MAC, serial, model, or firmware.

### Side observation

The device advertises **`_ssh._tcp` on port 22**. SSH was not exercised (no credentials supplied; out of spike scope), but this is an undocumented administrative surface worth a follow-up security review separate from the HA integration design.
...
The local ARP cache shows the DMP168 at `34:d0:b8:27:38:fc on en0` — sufficient to verify Task 4's OUI claim.

## Task 4 — OUI lookup

The web-GUI screenshot in `references/REVA1_DMP168_User_Manual.pdf` p. 24 shows MAC `34:D0:B8:27:2D:96`. The dev unit at `192.168.1.176` has MAC `34:D0:B8:27:38:FC` (per local ARP). Both fall in the same 28-bit block.

`api.maclookup.app/v2/macs/34D0B82738FC` returns:
