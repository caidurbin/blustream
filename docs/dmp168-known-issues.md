...
- **Root cause is unverified.** Every command surface (Telnet 23, raw 8000, web CGI) fails together while the static HTTP layer (port 80) and the SSH daemon (`dropbear_2022.83` on port 22) continue to serve normally — strong evidence that a single shared on-device component is the failing element, likely the serial-bridge daemon behind the `/tmp/web2ser` fifo.
...
2. **Ask the vendor** for (a) a firmware fix or workaround for the daemon-stability issue — the public documentation includes no errata, troubleshooting, watchdog, or daemon architecture notes — and (b) SSH credentials. The device runs `dropbear_2022.83` on port 22 and the SSH daemon stays up during the *problem* state, so shell access would enable direct diagnosis (`ps`, `lsof`, `/tmp/web2ser`) and likely a software-only recovery path. Attach `dmp168_monitor.log` to the inquiry — it records one full transition at 30-second resolution.
