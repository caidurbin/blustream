...
- Run `ruff check bluestream tests` and `ruff format` before committing
...
- New device types go under `bluestream/devices/<name>/` and subclass the base in `bluestream/base/`
- Connection transports (TCP, serial, ...) live under `bluestream/connection/`
- CLI commands belong in `bluestream/cli/`; keep device logic out of CLI modules
